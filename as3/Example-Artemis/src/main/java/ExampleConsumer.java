...
public class ExampleConsumer implements MessageListener {
	...
	public static void main(String[] args) {
		...
		try(ActiveMQConnectionFactory connectionFactory = new ActiveMQConnectionFactory("tcp://localhost:61616");
			...
			//Topic topic = session3.createTopic("LabTopic");
			//MessageConsumer consumer3 = session3.createConsumer(topic);
			//Message msg = consumer3.receive();
			//if (msg instanceof TextMessage) {
			//	TextMessage txt = (TextMessage) msg;
			//	System.out.println("Synchronous: " + txt.getText());
			//}
			//consumer3.setMessageListener(new ExampleConsumer());
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
